...
var processAddListener = process.addListener
var processRemoveListener = process.removeListener
var processOnce = process.once

process.addListener = process.on = function (ev, handler) {
  if (ev !== 'uncaughtException') {
    return processAddListener.apply(this, arguments)
  }

  processRemoveListener.call(process, 'uncaughtException', onUncaught)
  var ret = processAddListener.apply(this, arguments)
  if (process.listeners('uncaughtException').length === 0) {
    processAddListener.call(process, 'uncaughtException', onUncaught)
  }
  return ret
}

process.once = function (ev, handler) {
  if (ev !== 'uncaughtException') {
    return processOnce.apply(this, arguments)
  }

  processRemoveListener.call(process, 'uncaughtException', onUncaught)
  var ret = processOnce.apply(this, arguments)
  if (process.listeners('uncaughtException').length === 0) {
    processAddListener.call(process, 'uncaughtException', onUncaught)
  }
  return ret
...
process.removeListener = function (ev, handler) {
  if (ev !== 'uncaughtException') {
    return processRemoveListener.apply(this, arguments)
  }

  processRemoveListener.call(process, 'uncaughtException', onUncaught)
  var ret = processRemoveListener.apply(this, arguments)
  if (process.listeners('uncaughtException').length === 0) {
...
tap.on('end', function () {
  process.removeListener = processRemoveListener
  process.on = process.addListener = processAddListener
  process.once = processOnce
  processRemoveListener.call(process, 'uncaughtException', onUncaught)
})
